...
if [ ! -z "${APP_SERVICE}" ]; then
  ...
  if [ "${USE_ISTIO_GATEWAY}" = true ]; then
    ...
    IP_ADDR=$( kubectl get services --namespace ${CLUSTER_NAMESPACE} | grep ${APP_SERVICE} | awk '{print $4}' )
    PORT=$( kubectl get services --namespace ${CLUSTER_NAMESPACE} | grep ${APP_SERVICE} | awk '{print $5}' | sed 's/\([0-9]*\):.*/\1/g' )
  fi
  export APP_URL=https://${IP_ADDR}:${PORT} # using 'export', the env var gets passed to next job in stage
